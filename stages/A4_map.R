...
load (file.path (parsed.dir, 'trees_with_ages.RData'))
...
load (file.path (resolve.dir, 'preresolved.RData'))
cat ('\nMapping names to published trees and outputting for each study ....')
studies <- list.files (pglt.dir)
counter <- foreach (i=1:length (studies)) %dopar% {
  ...
  cat ('\n.... study [', study, '], [', i, '/', length (studies), ']',
       sep = '')
  # get names
  names <- read.delim (file.path (pglt.dir, study, 'names.txt'), header=TRUE,
